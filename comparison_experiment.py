...
def fit_predict_save(model: EvoTree | Id3Estimator, datasets: dict[str, tuple[pd.DataFrame, pd.Series, pd.DataFrame, pd.Series]],
    ...
    results_test[official_name][0].append(
        accuracy_score(datasets[name][3], prediction_test))
    results_test[official_name][1].append(
        confusion_matrix(datasets[name][3], prediction_test))

    prediction_train = model.predict(datasets[name][0])
    results_train[official_name][0].append(
        accuracy_score(datasets[name][1], prediction_train))
    results_train[official_name][1].append(
        confusion_matrix(datasets[name][1], prediction_train))
...
def save_results(results: dict[str, list[list, list, None | np.ndarray]], algorithm_type: str, type: str):
    ...
    with open(f"results/tables_{type}_{algorithm_type}.txt", "w+") as file:
        headers = ["Nazwa zbioru danych", "średnia dokładność",
                   "odchylenie standardowe", "dokładność minimalna", "dokładność maksymalna"]
        ...
        file.write("-" * (longest_results_name_length - len(headers[0])) + " ".join(
...
if __name__ == "__main__":
    ...
    def succession(population, genetic_operations_population, scores, genetic_operations_scores): return elite_succession(
    ...
    def do_loop(seed_start: int, seed_end: int,
                ...
                results_id3_train: dict[str,
                                        list[list, list, None | np.ndarray]]
                ) -> None:
        for seed_nr in range(seed_start, seed_end):
            seed(seed_nr)
            np.random.seed(seed_nr)

            model = EvoTree(POPULATION_SIZE, 5, leaf_probability,
                            MAX_ITERATIONS, reproductions[0], 0.4, 0.3, 0.4, succession)
            fit_predict_save(model, datasets, results_evo_test, results_evo_train,
                             "breast_cancer", "breast_cancer_wisconsin_diagnostic")

            model = EvoTree(POPULATION_SIZE, 20, leaf_probability,
                            MAX_ITERATIONS, reproductions[1], 0.8, 0.3, 0.0, succession)
            fit_predict_save(model, datasets, results_evo_test,
                             results_evo_train, "dry_bean", "dry_bean_dataset")

            model = EvoTree(POPULATION_SIZE, 20, leaf_probability,
                            MAX_ITERATIONS, reproductions[3], 0.4, 0.0, 0.4, succession)
            fit_predict_save(model, datasets, results_evo_test,
                             results_evo_train, "glass", "glass_identification")

            model = EvoTree(POPULATION_SIZE, 20, leaf_probability,
                            MAX_ITERATIONS, reproductions[0], 0.4, 0.0, 0.4, succession)
            fit_predict_save(model, datasets, results_evo_test,
                             results_evo_train, "lol", "high_diamond_ranked_10min")

            model = EvoTree(POPULATION_SIZE, 5, leaf_probability,
                            MAX_ITERATIONS, reproductions[1], 0.4, 0.3, 0.4, succession)
            fit_predict_save(model, datasets, results_evo_test,
                             results_evo_train, "wine", "wine")

            model = Id3Estimator(5, 1, False, False, 0.0, True)
            fit_predict_save(model, datasets, results_id3_test, results_id3_train,
                             "breast_cancer", "breast_cancer_wisconsin_diagnostic")

            model = Id3Estimator(20, 10, True, False, 0.3, False)
            fit_predict_save(model, datasets, results_id3_test,
                             results_id3_train, "dry_bean", "dry_bean_dataset")

            model = Id3Estimator(20, 1, False, False, 0.0, True)
            fit_predict_save(model, datasets, results_id3_test,
                             results_id3_train, "glass", "glass_identification")

            model = Id3Estimator(5, 10, False, True, 0.0, True)
            fit_predict_save(model, datasets, results_id3_test,
                             results_id3_train, "lol", "high_diamond_ranked_10min")

            model = Id3Estimator(5, 1, False, False, 0.0, False)
            fit_predict_save(model, datasets, results_id3_test,
                             results_id3_train, "wine", "wine")
    ...
    def get_base_results_dict(manager: multiprocessing.Manager) -> dict[str, list[list, list, None | np.ndarray]]:
        ...
        - list of resulting accuracies for each of the 25 algorithm executions
        - list of resulting confusion matrices for each of the 25 algorithm executions
    ...
    with multiprocessing.Pool(processes=5) as pool:
        pool.starmap(do_loop, [(5 * seed_nr, 5 * (seed_nr + 1), results_evo_test, results_id3_test,
                                results_evo_train, results_id3_train) for seed_nr in range(5)])

    results_evo_test, results_id3_test = change_results_dict_to_regular_form(
        results_evo_test, results_id3_test)
    results_evo_train, results_id3_train = change_results_dict_to_regular_form(
        results_evo_train, results_id3_train)
